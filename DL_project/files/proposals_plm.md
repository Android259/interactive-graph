...
-> Addressed by `preprocessing/plm_alignment.py` + `tests/test_esm3_alignment.py` +
...
Verified concrete divergence mechanisms on the actual data:
  ...
  - **Selenomethionine (MSE)**: present (as HETATM) in raw `GM2A_1g13.pdb1` and
    ...
    0 MSE records meant Voronota had *deleted* those residues, not normalized them:
    its `voronota_restrict_atoms("-use", "[-protein]")` excludes MSE, so GM2A lost 2
    residues and PITPNA 8, in the MIDDLE of the sequence. The matching count was
    produced by the extra-trim itself, and the order was shifted -- only 47 of 160
    GM2A nodes and 49 of 261 PITPNA nodes carried their own residue's embedding.
    Fixed at the source: `preprocessing/convert_mse_to_met.py` rewrites MSE as MET,
    both graphs were regenerated with voronota-js v1.29.4412 from
    `data/structures/mse_fixed/`, and node counts now equal FASTA lengths (162, 269).
    Everything downstream of those two graphs was rebuilt: geometric transformer
    nodes, the tensor cache, `data/esm3_input/`, and the RNA-BAnG and ESM3 v2
    embeddings. All per-protein row trimming is gone -- neither the extra-trim nor the
    drop-row column that briefly replaced it survives; the loader only strips BOS/EOS,
    and `tests/test_esm3_alignment.py` compares residues, not counts, to keep it that
    way. See `preprocessing/EXTERNAL_TOOLS.md` for tool versions and checkpoints.
...
- `preprocessing/plm_alignment.py` -- reusable count-invariant check (embedding rows vs
