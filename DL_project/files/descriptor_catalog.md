...
| `hydropathy_rim_match` | `hydropathy_rim × hbond` | полярность/гидрофобность УСТЬЯ кармана (не ядра — см. раздел 2) встречает H-донор/акцептор головы; мотивировано [protein_lipid_binding_family_literature.md](protein_lipid_binding_family_literature.md) — IP_trans/START/OSBP узнают полярную/заряженную голову именно на входе в карман |
| `elongation_shape_match` | `pocket_elongation × lipid npr1` | форма туннеля (трубка/чаша) встречает собственную вытянутость лиганда (PMI1/PMI3); мотивировано механизмом OSBP/ORP — специфичность там определяется диаметром/длиной туннеля против жёсткого кольца стерола |
...
**Что известно про идентичность.** Раздел 2 меряет η² по семье для каждого протеинового
дескриптора отдельно (Mantel против ESM3, nearest-neighbour-по-семье); для геометрии
рёбер этой же проверки долго не было. `analysis/edge_geometry_family_check.py` закрывает
это: та же `eta_squared` (`analysis/feature_identity_check.py`, переиспользована, не
переписана), 35 белков / 9 семей (пол `(k-1)/(n-1)` = **0.235**, как в разделе 2), но
объект другой — 25-мерный структурированный вектор не даёт одного числа на белок сам по
себе (разное число рёбер на карман), поэтому мера — mean/std каждой из 25 колонок по
всем направленным рёбрам ВНУТРИ кармана (`--protein_pockets_only` включён отдельно для
этого измерения, независимо от того, что сами `..._family_neutral*` бейзлайны этот флаг
не ставят — чтобы «геометрия кармана» здесь значила то же самое, что в разделе 2, а не
геометрию рёбер по всему белку). Топ и низ полной таблицы (`--out` пишет её целиком):

| колонка | η² (семья) | что это |
|---|---|---|
| `quat_0_std` | **0.888** | разброс 1-й компоненты относительного кватерниона по карману — САМЫЙ высокий η² во всём проекте, выше `pocket_sasa_share` (0.85, раздел 2) |
| `quat_2_std` | **0.805** | разброс 3-й компоненты кватерниона |
| `rbf_2.0Å_std` (ближайший RBF-центр) | 0.746 | разброс доли самых тесных контактов по карману |
| `dir_z_mean` | 0.706 | средняя проекция направления контакта на локальную Z-ось |
| `rbf_3.3Å_std` | 0.659 | |
| `dir_z_std` | 0.642 | |
| `rbf_2.0Å_mean` | 0.639 | |
| `quat_1_std` | 0.638 | |
| `quat_3_std` | 0.574 | |
| … | … | (полная таблица — 50 строк, 25 колонок × mean/std) |
| `rbf_20.7Å_mean`/`_std` | 0.086 | у самого дальнего края диапазона (2–22Å) |
| `rbf_22.0Å_mean`/`_std` | **0.085** | ниже пола 0.235 не опускается ни одна колонка, но эти — ближе всего |

**Вывод.** Разброс (std) компонент относительного кватерниона по карману — не средняя
ориентация, а именно её ГЕТЕРОГЕННОСТЬ внутри кармана — самый семейственный сигнал из
всех измеренных в проекте дескрипторов, включая раздел 2. Это согласуется с тем, что
взаимная ориентация соседних остатков — ровно то, чем структурная биология отличает один
fold от другого (упаковка спираль-спираль/спираль-лист/антипараллельные тяжи имеют
устойчиво разные распределения относительной ориентации); разные seed-фолды дают разную
СТЕПЕНЬ однородности этой упаковки, что и ловит std. RBF-разложение дистанции ведёт себя
неравномерно по диапазону: ближние центры (2.0–4.7Å) несут сильный семейный сигнал
(0.5–0.75), дальние (18–22Å) — почти на полу (0.08–0.17). `log_area`/`log_boundary_ratio`
— умеренно (0.11–0.52). Ни один из 25 столбцов не опускается настолько ниже пола, чтобы
считаться однозначно «семейно-нейтральным» в духе раздела 2, но диапазон широкий (0.085
до 0.888) — не все 25 чисел одинаково виновны, и кватернион (в первую очередь его std)
— основной подозреваемый, а не голословно, а по числу выше любого протеинового
дескриптора раздела 2.

`--protein_edge_rbf_count=<N>` (число RBF-корзин, по умолчанию 16),
`--protein_edge_orientation_scalar` (кватернион → 1 скаляр, cos угла между локальными
Z-осями, `relative_rotation[..., 2, 2]`) и `--protein_edge_raw3` (только 3 сырых
скаляра distance/area/boundary через тот же EdgeMLPConv/EdgeAttentionConv, без RBF/
направления/ориентации вовсе) — три независимых флага, позволяющих проверить, снимает
ли урезание каждого из этих компонентов измеренную зависимость от семьи, не трогая
остальные два. `training/read_configuration.py::ModelConfig.validate` требует, чтобы
хотя бы один из `--protein_edge_attention`/`--protein_edge_mlp` уже стоял, и не даёт
`--protein_edge_raw3` сочетаться с двумя другими (оно и так их обнуляет).
