...
> **Пометка 2026-09-12**: §A ниже называет `..._lcs_esm3_balanced_lipid_classes_advprot`
> "текущим лучшим кандидатом" (строка "Третий пробел"). Трёхосевая проверка
> (честная метрика / GAP / стабильность обучения) против `liphid32` и других
> сиблингов — в [lcs_geometric_edge_best_candidate_recheck.md](lcs_geometric_edge_best_candidate_recheck.md);
> вывод неоднозначный (разные оси называют разных победителей), не "advprot лучше
> безусловно". Описание самого пробела в null-model-тулинге (§A) остаётся верным.
