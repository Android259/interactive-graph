...
The screen reports 658 such (protein, lipid) combinations. Each carries the screen or screens the
species was recovered in, and 28 were recovered in both.

The panel comprises 35 proteins in nine families and 312 lipid species in 34 head-group classes
...
\hline
CRAL-TRIO & 9 & 167 & RLBP1 63, TTPAL 63, SEC14L6 14, TTPA 10, SEC14L2 9, SEC14L4 3, ATCAY 2, SEC14L5 2, BNIPL 1 \\
START & 3 & 154 & STARD2 68, STARD10 65, STARD11 21 \\
lipocalin & 10 & 87 & LCN1 27, FABP1 24, FABP5 10, PMP2 9, FABP7 8, LCN15 5, CRABP2 1, RBP1 1, RBP4 1, RBP5 1 \\
GLTP & 2 & 80 & GLTPD1 41, GLTP 39 \\
...
The negative class is built as the complement. The panel defines $35 \times 312 = 10920$
combinations, and the 10262 that were not reported bound receive the label zero. The training
table is therefore the complete grid, one row per combination, at a positive rate of 6.0\%. Its
...
profile at 0.335 against 0.259 for ignoring the protein, thirteen cavity descriptors reach 0.284
and four shape numbers 0.228, correlating with the profile at $+0.094$ and $-0.029$. Appending the
thirteen to ESM3 lowers it to 0.291. Their nearest-neighbour family rate is 0.51 and 0.63
...
\hline
ignore the protein & -- & -- & -- & -- & -- & 0.167 \\
ESM3 & 1536 & 0.943 & 0.974 & 0.905 & $+0.227$ & \textbf{0.194} \\
ESM-IF1 & 512 & 0.800 & 0.762 & 0.256 & $+0.127$ & 0.152 \\
ProteinMPNN & 128 & 0.857 & 0.994 & 0.958 & $+0.103$ & 0.165 \\
\hline
\end{tabular}
...
canonicalisation. Of the 10920 rows, 7909 (72.4\%) carry more than one candidate. Phosphatidyl\-
...
A row is encoded by one of its candidates, drawn afresh each time the row is presented. Over an
epoch a species is therefore shown as its whole candidate set, not as one arbitrary member of it.
Evaluation marginalises over the same set, scoring the row once per candidate and averaging the
probabilities before the threshold, so that the number reported answers for the species and not for
a structure the spectrometer never singled out.
...
structure with. Token counts range from 24 to 132 over the 1226 structures.
...
two per positive within each protein; these rows are the pool. That leaves about one third of it
positive and equalises the positive rate across proteins, so no protein can be answered by its own
base rate. The rows a split holds out of the pool are its block, halved into validation and test.
A seed changes two things and nothing else: which zeros are drawn, and how the block is halved.
Which proteins and which lipid classes leave training is fixed. Every figure below is averaged
over five seeds.
...
training. The reference predictor accordingly reaches 0.588 balanced accuracy on test averaged
...
Results obtained under this split are to be read against 0.59, not 0.50.
...
\hline
sphingolipids & 0.458 & 84 (12.8\%) & sphingoid backbone, all of it \\
phosphorus-free & 0.553 & 61 (9.3\%) & neutral glycerolipids, free fatty acids, retinol \\
choline & 0.653 & 227 (34.5\%) & phosphocholine head, di- and lyso- \\
anionic & 0.766 & 188 (28.6\%) & anionic glycerophospholipid heads \\
\hline
\end{tabular}
...
in decreasing order of $s$ until they cover a set share of the family's positives. Rows of the
family in the classes that stayed are discarded rather than evaluated: their protein is absent, so
they cannot train, and they are the only rows a frequency lookup could still score.
...
\hline
CRAL-TRIO & 0.854 & 0.902 & 0.902 & 0.796 & \textbf{0.784} & 0.761 & 0.803 \\
GLTP & 0.728 & 0.706 & 0.697 & 0.697 & \textbf{0.697} & 0.567 & 0.574 \\
IP\_trans & 0.827 & 0.827 & 0.827 & 0.827 & \textbf{0.827} & 0.846 & 0.846 \\
LBP\_BPI\_CETP & 0.827 & 0.827 & 0.827 & 0.827 & \textbf{0.827} & 0.827 & 0.671 \\
lipocalin & 0.890 & 0.795 & 0.658 & 0.658 & \textbf{0.658} & 0.698 & 0.698 \\
scp2 & 0.829 & 0.908 & 0.928 & 0.928 & \textbf{0.928} & 0.878 & 0.878 \\
START & 0.828 & 0.828 & 0.828 & 0.654 & \textbf{0.654} & 0.654 & 0.693 \\
\hline
mean & 0.826 & 0.828 & 0.810 & 0.770 & \textbf{0.768} & 0.747 & 0.738 \\
positives left in training, lowest & 378 & 378 & 343 & 307 & \textbf{307} & 256 & 142 \\
\hline
\end{tabular}
...
similarity of 0.768 and still leaves every family at least 307 positives to train on; beyond it
the gain is small and the cost is not. One threshold serves all families even though their optima
differ. scp2 is the one to read with care: it stays near 0.9 wherever the threshold is put, so its
...
\hline
CRAL-TRIO & 8 & 136 & 121 & 1110 & 309 & 309 \\
GLTP & 3 & 65 & 1 & 1731 & 66 & 67 \\
IP\_trans & 3 & 47 & 207 & 1179 & 331 & 331 \\
LBP\_BPI\_CETP & 3 & 45 & 188 & 1251 & 296 & 295 \\
lipocalin & 15 & 72 & 228 & 1029 & 403 & 403 \\
scp2 & 9 & 36 & 143 & 1416 & 264 & 263 \\
START & 5 & 133 & 197 & 921 & 460 & 459 \\
\hline
\end{tabular}
...
Two properties of these blocks matter when per-family results are read. Removing the sphingolipids
costs training a single positive, the one sphingomyelin LCN1 takes. The model therefore reaches the
GLTP block having seen one positive sphingolipid in all, and that block is also the smallest, at 67
rows. Second, a protein whose positives all lay in the removed classes
contributes nothing to training yet still appears in the block, through its own rows in those
classes. For lipocalin this is four of the 25 proteins outside the held-out family.

The two operations have to be ordered with care. Removing classes cuts across proteins, so a
protein whose positives lay in a removed class loses nearly all of its positives and almost none
of its negatives. Sampling negatives over the whole table and cutting afterwards produced proteins
with four positives against 91 negatives. It also produced proteins that entered training with a
single label value, which no reweighting can repair. Negatives are therefore matched to positives
separately within each side of the coming split. After that no protein is one-sided and no row is
discarded.
...
\hline
protein families held out & 0.588 & 99.9\% \\
two axes, familiar rows retained & 0.506 & 9\% \\
two axes & 0.500 & 0\% \\
