...
def parse_report(path: Path, reports_root: Path) -> dict[str, object]:
    ...
    config["architecture"] = relative.parts[0]
...
def configuration_label(reports: list[dict[str, object]]) -> str:
    ...
    parts = []
    architectures = sorted({config.get("architecture", "") for config in configs})
    if len(architectures) == 1 and architectures[0]:
        parts.append(architectures[0])
    weight_labels = []
...
def plot_aggregates(
    ...
    ordered = sorted(
        aggregates,
        key=lambda item: (
            _group_sort_key(",".join(str(group) for group in item["groups"])),
            str(item["subgroup"]),
        ),
    )
    ...
    group_names = [
        ",".join(str(group) for group in item["groups"])
        for item in ordered
    ]
