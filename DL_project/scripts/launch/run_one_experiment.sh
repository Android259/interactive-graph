...
# The alternation in GPU_MODEL_GLOB ("*A100*|*V100*") has to be split by hand: it
# arrives in a variable, where `|` is an ordinary character, so matching the
# whole string as one pattern would reject every real GPU.
gpu_supported=0
...
# Slot 0's lock path is the one a packed job also takes for its first slot, so a
# packed job and an unpacked one exclude each other instead of quietly sharing
# the card.
