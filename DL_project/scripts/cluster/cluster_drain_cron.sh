...
# the whole point: getting jobs onto the cluster does not depend on any laptop
# being switched on or connected. The queue itself lives on the cluster too
# (.<cluster>_job_queues/), so nothing about submission is remote.
#
#   CLUSTER_NAME=bigfoot bash scripts/cluster/cluster_drain_cron.sh
#
# scripts/wait_and_sync.sh installs the crontab entry that calls this; running
...
QUEUE_HELPER="${QUEUE_HELPER:-${PROJECT_DIR}/scripts/cluster/cluster_queue_remote.sh}"
