# DeepCLIP, lipid-isolation point within CRAL-TRIO (cral-trio__0.56). Fills in the Tanimoto-similarity-vs-metric curve
...
# --lipid_isolation=cral-trio__0.56 -- a species set chosen for CRAL-TRIO's own panel -- against
...
#
# --lipid_isolation=cral-trio__0.56 holds 89 lipid species out of training for CRAL-TRIO
...
# isolation 0.555 -- the GLOBAL blocks calibrated against the whole table do not
...
# --family_only value).
#
# Composition: hexosyl ceramide 22%, sphingomyelin 19%, ceramide 16%, ceramide phosphate 11%, +7 more classes. the floor of what CRAL-TRIO's own panel supports at this block size; the noisiest rung -- seed-to-seed isolation swings 0.53-0.68 once the sampler draws negatives (measured, seeds 0-4). Reconstructed at seeds 0-4 with this file's own
# sampler (15 positives on the full table): test block 22 rows, 7 positives at every seed. Isolation is
...
--lipid_isolation=cral-trio__0.56
