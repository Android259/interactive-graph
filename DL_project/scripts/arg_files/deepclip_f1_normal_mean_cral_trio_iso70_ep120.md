# DeepCLIP, lipid-isolation point within CRAL-TRIO (cral-trio__0.70). Fills in the Tanimoto-similarity-vs-metric curve
...
# --lipid_isolation=cral-trio__0.70 -- a species set chosen for CRAL-TRIO's own panel -- against
...
#
# --lipid_isolation=cral-trio__0.70 holds 90 lipid species out of training for CRAL-TRIO
...
# isolation 0.700 -- the GLOBAL blocks calibrated against the whole table do not
...
# --family_only value).
#
# Composition: phosphatidylcholine 83%, sphingomyelin 12%, lysophosphatidylcholine 4%. close to iso65's nominal target but a cleaner, PC-dominated block -- kept as its own rung because the two measure at visibly different x once seed noise settles (0.651 vs 0.686). Reconstructed at seeds 0-4 with this file's own
# sampler (21 positives on the full table): test block 32 rows, 11 positives at every seed. Isolation is
...
--lipid_isolation=cral-trio__0.70
