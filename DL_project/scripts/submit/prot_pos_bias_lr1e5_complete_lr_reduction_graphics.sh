...
mkdir -p script_logs/prot_pos_bias_lr1e5_complete_lr_reduction_graphics
...
submit_variant() {
    ...
    local variant="$3"
    local lr="$4"
    local output_dir="script_logs/prot_pos_bias_lr1e5_complete_lr_reduction_graphics/${group}"
...
printf 'Submitted 10 lr=1e-5 jobs for the lr_reduction graphics grid; lr=3e-5 is already complete for these groups and seeds.\n'
