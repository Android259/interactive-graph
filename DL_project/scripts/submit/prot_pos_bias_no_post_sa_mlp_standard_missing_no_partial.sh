...
mkdir -p script_logs/prot_pos_bias_no_post_sa_mlp_standard_missing_no_partial

submit_variant() {
    local group="$1"
    local seed="$2"
    local variant="no_post_sa_mlp_standard"
    local output_dir="script_logs/prot_pos_bias_no_post_sa_mlp_standard_missing_no_partial/${group}"
    ...
    oarsub \
        --name "nopost_missing_${group}_s${seed}" \
...
for specification in "${jobs[@]}"; do
    IFS="|" read -r group seed <<< "${specification}"
    submit_variant "${group}" "${seed}"
done

printf 'Submitted 24 no-post-SA-MLP missing non-partial jobs.\n'
