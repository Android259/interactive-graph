...
mkdir -p script_logs/prot_pos_bias_final_dropout_wd1e5_seeds234

submit_variant() {
    local group="$1"
    local seed="$2"
    local variant="$3"
    local final_dropout="$4"
    local output_dir="script_logs/prot_pos_bias_final_dropout_wd1e5_seeds234/${group}"
    ...
    oarsub \
        --name "pbfd234_${group}_${variant}_s${seed}" \
...
for group in "${groups[@]}"; do
    for seed in "${seeds[@]}"; do
        for specification in "${variants[@]}"; do
            IFS="|" read -r variant final_dropout <<< "${specification}"
            submit_variant "${group}" "${seed}" "${variant}" "${final_dropout}"
        done
    done
done

printf 'Submitted 54 final-dropout jobs with weight_decay=1e-5 across all 9 groups and seeds 2/3/4.\n'
