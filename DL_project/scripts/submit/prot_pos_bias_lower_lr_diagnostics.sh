...
mkdir -p script_logs/prot_pos_bias_lower_lr_diagnostics
...
submit_variant() {
    ...
    local variant="$3"
    local lr="$4"
    local output_dir="script_logs/prot_pos_bias_lower_lr_diagnostics/${group}"
    ...
    oarsub \
        --name "pblr_${group}_${variant}_s${seed}" \
...
printf 'Submitted 20 lower-LR diagnostic jobs across 5 groups and seeds 0/1.\n'
