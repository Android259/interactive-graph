...
mkdir -p script_logs/prot_pos_bias_standard_class_weights_seeds34

submit_variant() {
    local group="$1"
    local seed="$2"
    local variant="class_weights"
    local output_dir="script_logs/prot_pos_bias_standard_class_weights_seeds34/${group}"
...
printf 'Submitted 18 standard class-weight jobs across all 9 groups and seeds 3/4.\n'
