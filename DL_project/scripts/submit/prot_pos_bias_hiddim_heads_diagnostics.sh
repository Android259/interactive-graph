...
mkdir -p script_logs/prot_pos_bias_hiddim_heads_diagnostics
...
submit_variant() {
    ...
    local hiddim="$4"
    local heads="$5"
    local output_dir="script_logs/prot_pos_bias_hiddim_heads_diagnostics/${group}"
    ...
    oarsub \
        --name "pbhh_${group}_${variant}_s${seed}" \
...
for group in "${groups[@]}"; do
    for seed in "${seeds[@]}"; do
        for specification in "${variants[@]}"; do
            IFS="|" read -r variant hiddim heads <<< "${specification}"
            submit_variant "${group}" "${seed}" "${variant}" "${hiddim}" "${heads}"
        done
    done
done

printf 'Submitted 20 hiddim/heads diagnostic jobs across 5 groups and seeds 0/1.\n'
