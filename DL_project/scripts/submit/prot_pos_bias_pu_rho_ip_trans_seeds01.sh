...
mkdir -p script_logs/prot_pos_bias_pu_rho_ip_trans_seeds01
...
submit_variant() {
    ...
    local rho="$3"
    local variant="$4"
    local output_dir="script_logs/prot_pos_bias_pu_rho_ip_trans_seeds01/${group}"
    ...
    oarsub \
        --name "pu_${group}_${variant}_s${seed}" \
...
seeds=(0 1)
variants=(
    "protein_inverse_pu_rho005|0.05"
    "protein_inverse_pu_rho010|0.10"
    "protein_inverse_pu_rho020|0.20"
    "protein_inverse_pu_rho030|0.30"
)
...
printf 'Submitted 8 protein-inverse PU rho jobs for IP_trans across seeds 0/1.\n'
