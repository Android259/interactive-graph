...
mkdir -p script_logs/prot_pos_bias_protein_inverse_seeds34

submit_variant() {
    local group="$1"
    local seed="$2"
    local variant="inverse"
    local output_dir="script_logs/prot_pos_bias_protein_inverse_seeds34/${group}"
...
printf 'Submitted 18 inverse protein-class-weight jobs across all 9 groups and seeds 3/4.\n'
