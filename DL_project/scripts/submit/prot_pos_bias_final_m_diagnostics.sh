...
mkdir -p script_logs/prot_pos_bias_final_m_diagnostics
...
submit_variant() {
    ...
    local variant="$3"
    local final_m="$4"
    local output_dir="script_logs/prot_pos_bias_final_m_diagnostics/${group}"
    ...
    oarsub \
        --name "pbfm_${group}_${variant}_s${seed}" \
...
for group in "${groups[@]}"; do
    for seed in "${seeds[@]}"; do
        for specification in "${variants[@]}"; do
            IFS="|" read -r variant final_m <<< "${specification}"
            submit_variant "${group}" "${seed}" "${variant}" "${final_m}"
        done
    done
done

printf 'Submitted 36 final-layer width diagnostic jobs across all 9 groups and seeds 0/1.\n'
