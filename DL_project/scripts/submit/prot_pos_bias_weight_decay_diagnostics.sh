...
mkdir -p script_logs/prot_pos_bias_weight_decay_diagnostics
...
submit_variant() {
    ...
    local output_dir="script_logs/prot_pos_bias_weight_decay_diagnostics/${group}"
    ...
    oarsub \
        --name "pbwd_${group}_${variant}_s${seed}" \
...
for group in "${groups[@]}"; do
    for seed in "${seeds[@]}"; do
        for specification in "${variants[@]}"; do
            IFS="|" read -r variant weight_decay <<< "${specification}"
            submit_variant "${group}" "${seed}" "${variant}" "${weight_decay}"
        done
    done
done

printf 'Submitted 36 weight-decay diagnostic jobs across all 9 groups and seeds 0/1.\n'
