...
- CSV/artifact names расходятся для четырёх proteins:
  `RBP1→RET1`, `RBP4→RET4`, `RBP5→RET5`, `STARD11→CERT`.
- ESM3 v1 требует дополнительного trim для `GM2A` (1 пара) и `PITPNA`
  (4 пары), затем общего удаления BOS/EOS.
- RET4 несогласован: 174 graph nodes, 175 pocket residues и edge endpoint 175,
  отсутствующий в node table. Loader сейчас преобразует неизвестный residue ID в
  node 0; это необходимо исправлять отдельной версией данных, поскольку graph и
  результат модели изменятся.
...
Создан `data/protein_registry.csv`:

```text
protein_id,artifact_stem,family,uniprot_id,esm3_v1_extra_trim_pairs
```

Registry объединяет:

- CSV protein ID;
- graph/embedding stem;
- protein family;
- UniProt ID;
- исторический ESM3 v1 trim.

Active `protein_graph_builder.py` и `plm_alignment.py` читают registry через
`dataloader/protein_registry.py`. Runtime-словари `familydic`, `Gene_uni`, rename
map и отдельный `EXTRA_TRIM_PAIRS` для active path больше не нужны.
...
8. Scientific corrections, включая RET4, выпускать отдельно как
